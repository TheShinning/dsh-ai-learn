...
function tiffSize(bytes            )                                      {
  const little = startsWith(bytes, [0x49, 0x49])
  if (!little && !startsWith(bytes, [0x4d, 0x4d])) return {}
  const read16 = (offset        )                     => (little ? u16le(bytes, offset) : u16be(bytes, offset))
  const read32 = (offset        )                     => {
...
export function probeImageSize(bytes            )                                      {
  try {
 ...
 */
export function readImageBytes(bytes            , reference        )                    {
