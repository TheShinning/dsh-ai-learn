...
export const CLASSROOM_GAP_LABEL                                   = {
...
function withinWindow(item          , from        , to        )          {
...
export function findGaps(input   
  ...
  const gaps                 = []
  const nodeTitle = (nodeId        )                     => input.nodes.find((node) => node.id === nodeId)?.title
  ...
  const cardNodeIds = new Set((input.cards ?? []).map((card) => card.nodeId).filter(Boolean)            )
  for (const [nodeId, items] of byNode) {
...
export function buildClassroomRecord(input                      )                  {
  ...
  const nodesTouched                       = [...byNode.entries()]
    .map(([nodeId, items]) => ({
      nodeId,
...
export function renderClassroomRecord(record                 )         {
  const head           = [
    `# 课堂记录 · ${record.title}`,
    '',
 ...
 */
export function classroomRecordPath(record                 , dir = '.study/records')         {
