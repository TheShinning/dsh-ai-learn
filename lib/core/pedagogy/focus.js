...
import { buildMasteryIndex, prerequisitesReady } from '../knowledge/graph.js'
...
export const FOCUS_REASON_LABEL                              = {
  confused: '困惑点优先澄清',
  'due-review': '到期复习',
...
const CONTAINER_TYPES                                     = new Set(['textbook', 'chapter'])
...
/** 图里可作为学习单元的节点。 */
export function studyNodes(graph                )                           {
...
export function selectFocusNode(
  ...
  const masteryOf = (node               )          => {
    const value = index[node.id]
    return value === undefined ? 'unknown' : value
  }
  const evidenceOf = (node               )         => byNode.get(node.id)?.length ?? 0

  const build = (reason             , node               , extra                                                    , directive        )                => ({
    reason,
    nodeId: node.id,
    title: node.title,
...
export function focusReasonDirective(reason             )         {
  switch (reason) {
    case 'confused':
...
export function renderFocus(decision               )         {
