...
import { STUDY_DOMAIN, STUDY_DOMAIN_NAME,                 } from './storage.js'
import { createDomainStore, createMemoryStore,                   } from './store.js'
import { createStudyTools } from './tools.js'
import { decodeSourceFormat } from '../core/pedagogy/material.js'
import { TEACHING_MODE_LABEL, materialDirective, strategyFor } from '../core/pedagogy/strategy.js'
import { MATERIAL_TYPE_LABEL } from '../core/types.js'
...
function resolveStore(ctx         )                                                                {
  ...
  ctx.effect(async () => {
    try {
      const opened = (await storage.domain .open(STUDY_DOMAIN))                
      handle = opened
 ...
 * 既浪费又会让每次调用都重新遍历域表。
 */
function createDomainStoreLazy(getHandle                                )             {
  ...
  return new Proxy(memory, {
    get(target, property, receiver) {
      ...
      if (handle) {
        ...
        const value = Reflect.get(cached.store          , property)
...
export function apply(ctx         , config        )       {
  ...
  ctx.effect(() => {
    const disposers = createStudyTools(store, ctx                               ).map((tool) => ctx.tools.register(tool))
    return () => {
