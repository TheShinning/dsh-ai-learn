...
import { EvidenceIdAllocator, evidenceKey,               } from '../core/knowledge/evidence.js'
import {
  buildMasteryIndex,
  buildRouteStops,
  masteryStats,
...
import { deriveMastery, masteryWithout, positiveKindCount } from '../core/knowledge/mastery.js'
import { buildStudyStats, renderStats } from '../core/growth/stats.js'
import {
  CARD_KIND_LABEL,
  impracticableReason,
  isPracticable,
...
} from '../core/review/cards.js'
import { SRS_RESULT_LABEL, isDue, scheduleReview, srsMasterySignal,                } from '../core/review/srs.js'
import { deriveProbeState } from '../core/pedagogy/probe.js'
import { renderMove, nextSocraticMove,                                     } from '../core/pedagogy/step.js'
import { answerToEvidence, quizIntent, quizDirective, QUIZ_INTENT_LABEL,                 } from '../core/pedagogy/quiz.js'
import { renderFocus, selectFocusNode,                    } from '../core/pedagogy/focus.js'
import {
  classifyMaterial,
  decodeSourceFormat,
  encodeSourceFormat,
  ...
  shouldAskUser,
} from '../core/pedagogy/material.js'
import { materialDirective, strategyFor, teachingModeOf, TEACHING_MODE_LABEL } from '../core/pedagogy/strategy.js'
import { buildReviewPlan, renderReviewPlan, shouldInterrupt, INTERRUPT_MAX_AGAIN } from '../core/review/plan.js'
import { capabilities } from '../ingest/index.js'
import { INGEST_TIER_LABEL } from '../core/ingest/types.js'
import { applyImportPlan, planTextbookImport, textbookKey } from '../core/textbook/identity.js'
import {
  CLASSROOM_GAP_LABEL,
...
} from '../core/records.classroom.js'
import { buildStructureGraph, describeStructure, splitTextbookStructure } from '../core/textbook/structure.js'
import { BLOOM_LABEL, MASTERY_LABEL, MATERIAL_TYPE_LABEL, normalizeBloomLevel,                                                                        } from '../core/types.js'
import { ingestFile } from '../ingest/index.js'
import { buildStudyContext, teachingStateSnapshot } from './prompt.js'
                                                                              
import { newCardSrs } from './store.js'

const text = (value        ) => [{ type: 'text'         , text: value }]

/** 组装某本教材的完整图谱视图。 */
async function graphOf(store            , textbookKey         )                                                   {
...
function latestErrorType(evidence                     , nodeId         )                        {
  let latest                      
 ...
 */
async function localAdviceHint(format        )                  {
  try {
    const list = await capabilities()
    const relevant = list.filter((item) => item.formats.includes(format         ))
 ...
 */
export function createStudyTools(store            , ctx                   ) {
  ...
  const rebuild = defineTool({
    ...
    async execute(args                           ) {
      ...
      const existing                 = { nodes, edges, evidence }
  ...
  const formats = defineTool({
    ...
    async execute() {
      ...
      for (const tier of ['native', 'builtin', 'external']         ) {
    ...
    async execute(args                                          ) {
      ...
      try {
        ...
        if (fs?.writeText) {
          ...
          const absolute = String((target                     )?.path ?? '')
