...
import type { SourceFormat } from '../ingest/types.ts'
import type { MaterialType } from '../types.ts'
import { MATERIAL_TYPE_LABEL } from '../types.ts'

/** 分类置信度。`explicit` 来自用户明确指定，`unknown` 表示没有可用特征。 */
export type MaterialConfidence = 'explicit' | 'strong' | 'weak' | 'unknown'

export const MATERIAL_CONFIDENCE_LABEL: Record<MaterialConfidence, string> = {
  explicit: '用户明确指定',
  strong: '特征明显',
  weak: '特征较弱（可被一句改判）',
  unknown: '无可用特征',
...
export type MaterialClassification = {
  ...
  readonly confidence: MaterialConfidence
  /** 为什么这么判（命中/未命中的可读依据）。 */
  readonly why: string
  /** 命中的特征数（用于解释与调试）。 */
...
export type MaterialInput = {
  ...
  readonly sourceFormat?: SourceFormat | string
  readonly markdown: string
  /** 用户明确指定的类型：一旦给出就是 `explicit`，不再猜。 */
 ...
 */
export function encodeSourceFormat(format: string, type: MaterialType, confidence: MaterialConfidence): string {
  return `${format}|${type}|${confidence}`
}

export type DecodedSourceFormat = {
...
export function decodeSourceFormat(value: string | undefined): DecodedSourceFormat {
  if (!value) return {}
...
const SIGNATURES: Record<MaterialType, Signature> = {
  regulation: {
...
const TITLE_SIGNATURES: Record<MaterialType, readonly RegExp[]> = {
 ...
 */
export function classifyMaterial(input: MaterialInput): MaterialClassification {
  ...
  const score = new Map<MaterialType, number>()

  for (const [type, patterns] of Object.entries(TITLE_SIGNATURES) as [MaterialType, readonly RegExp[]][]) {
  ...
  for (const [type, signature] of Object.entries(SIGNATURES) as [MaterialType, Signature][]) {
  ...
  const confidence: MaterialConfidence = strongEnough ? 'strong' : 'weak'
 ...
 */
export function shouldAskUser(classification: MaterialClassification): boolean {
  return classification.confidence === 'weak' || classification.confidence === 'unknown'
}

/** 把分类结果渲染成一行（供工具与命令复用）。 */
export function renderClassification(classification: MaterialClassification): string {
