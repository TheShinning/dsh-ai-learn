...
import type { ProbeState } from './probe.ts'
import { depthDirective, toneDirective } from './probe.ts'
...
export const SOCRATIC_STEP_LABEL: Record<SocraticStep, string> = {
  diagnose: '定向',
  probe: '追问',
  scaffold: '降阶',
...
export const SOCRATIC_STEP_ORDER: readonly SocraticStep[] = [
  'diagnose',
  'probe',
  'scaffold',
  'explain-and-retell',
...
export const SOCRATIC_ACTION_LABEL: Record<SocraticAction, string> = {
...
export type QuizOutcome = 'retell-ok' | 'retell-fail' | 'again' | 'hard' | 'good'
...
export type SocraticMoveInput = {
  readonly nodeId?: string
  /** 由 `masteryFromEvidence()` 算出的掌握度（本模块不自算，避免两处判定）。 */
  readonly mastery: EvidenceTone | 'unknown'
  readonly probeState: ProbeState
  /** 该节点的证据（用于复述/测验的既有结果与降阶梯度）。 */
  readonly evidence?: readonly Evidence[]
  /** 该节点相关的卡片。 */
  readonly cards?: readonly CardLike[]
  /** 本轮这一小节里，刚发生的一次测试或复述的结果。 */
  readonly lastQuiz?: QuizOutcome
}

/** 复述与测验的既有结果，从证据里数出来（不靠调用方传"我记得学生复述过了"）。 */
function priorOutcomes(evidence: readonly Evidence[] | undefined): {
  readonly retold: boolean
  readonly retellFailed: boolean
  readonly quizPassed: boolean
  readonly quizFailed: boolean
} {
 ...
 * 8. 其余 → `ask`（一次一个关键问题）。
 */
export function nextSocraticMove(input: SocraticMoveInput): SocraticMove {
  const base = {
    questionBudget: 1 as const,
    scaffoldLadder: [] as readonly string[],
  }
  ...
  const tone: EvidenceTone = input.mastery
 ...
 */
export function renderMove(move: SocraticMove): string {
  const parts = [
...
export function appendStep(
  steps: readonly { readonly step: SocraticStep; readonly at: string; readonly action: SocraticAction }[],
  move: SocraticMove,
  at: Date,
): readonly { readonly step: SocraticStep; readonly at: string; readonly action: SocraticAction }[] {
