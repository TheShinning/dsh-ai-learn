 ...
 */
export const TEACHING_MODE_INVARIANTS: readonly string[] = [
  '一次只问一个问题',
...
export type TeachingStrategy = {
  ...
  /** 同档内的焦点偏好（`selectFocusNode()` 的 `preferNodeIds` 由此派生；不改变档位顺序）。 */
  readonly focusBias: readonly string[]
  /** 追问的重点。 */
  readonly questionBias: readonly string[]
  /** 出题的形态偏好。 */
  readonly cardBias: readonly CardKind[]
  /** 降阶阶梯（固定顺序，模型不得自选）。 */
  readonly scaffoldLadder: readonly string[]
  /** 收束时必须检查的项。 */
  readonly wrapUpChecklist: readonly string[]
}
...
const STRATEGIES: Record<MaterialType, TeachingStrategy> = {
  regulation: {
    mode: 'clause-driven',
...
export const FALLBACK_STRATEGY: TeachingStrategy = STRATEGIES.textbook
...
export function renderStrategy(strategy: TeachingStrategy): string {
 ...
 */
export function materialDirective(type: MaterialType): string {
