...
import type { BloomLevel, ErrorType, EvidenceTone, Mastery } from '../types.ts'
import { BLOOM_LABEL, bloomAtLeast } from '../types.ts'
import type { Evidence, SrsResult } from '../knowledge/evidence.ts'
import type { CardKind } from '../review/cards.ts'
import { CARD_KIND_LABEL } from '../review/cards.ts'

/** 这一步要学生做什么。 */
export type QuizIntent = 'explain-in-own-words' | CardKind

export const QUIZ_INTENT_LABEL: Record<QuizIntent, string> = {
  'explain-in-own-words': '用自己的话讲一遍',
  ...CARD_KIND_LABEL,
}

/** 作答的性质：普通回答 / 复述 / 卡片自评。 */
export type AnswerKind = 'answer' | 'retell' | 'card'

export type AnswerInput = {
  readonly nodeId: string
  readonly sourceId: string
  /** 学生原话（只取前若干字符做摘要，不整段入库）。 */
  readonly answer: string
  readonly kind?: AnswerKind
  /** 选择题的判定结果（来自 `judge()`）；不传表示"未作答"。 */
  readonly correct?: boolean
  /** 非选择题的自评（again / hard / good）。 */
  readonly selfReport?: SrsResult
  readonly bloomLevel?: BloomLevel
  /** 显式错误类型（由调用方判定时优先采用，否则按信号推断）。 */
...
function clip(text: string, limit = ANSWER_SUMMARY_LIMIT): string {
 ...
 */
export function answerToEvidence(input: AnswerInput): Pick<
  Evidence,
  'nodeId' | 'kind' | 'sourceId' | 'summary' | 'result' | 'correct' | 'mastery' | 'bloomLevel' | 'errorType' | 'createdAt'
> {
  const kind: AnswerKind = input.kind ?? 'answer'
...
export function retellEvidence(
  input: Omit<AnswerInput, 'kind'> & { readonly passed: boolean },
): ReturnType<typeof answerToEvidence> {
  return answerToEvidence({ ...input, kind: 'retell', correct: input.passed })
}

function correctOf(input: AnswerInput): boolean | undefined {
...
function toneOf(input: AnswerInput): EvidenceTone {
 ...
 */
function resultOf(input: AnswerInput, kind: AnswerKind, tone: EvidenceTone): string {
 ...
 */
export function inferErrorType(input: Pick<AnswerInput, 'answer' | 'correct' | 'selfReport'>): ErrorType | undefined {
...
function defaultSummary(input: AnswerInput, kind: AnswerKind): string {
...
export function quizDirective(intent: QuizIntent, mastery: Mastery): string {
  switch (intent) {
...
export function renderQuizIntent(intent: QuizIntent, mastery: Mastery): string {
