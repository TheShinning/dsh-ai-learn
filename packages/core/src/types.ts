...
export type Mastery = 'unknown' | 'partial' | 'confused' | 'mastered'

export const MASTERY_VALUES: readonly Mastery[] = ['unknown', 'partial', 'confused', 'mastered']
...
export function isMastery(value: unknown): value is Mastery {
  return typeof value === 'string' && (MASTERY_VALUES as readonly string[]).includes(value)
}

export function normalizeMastery(value: unknown, fallback: Mastery = 'unknown'): Mastery {
  return isMastery(value) ? value : fallback
}

/** 是否达到"已点亮"。 */
export function isLit(mastery: Mastery): boolean {
 ...
 * 但**输出永远是 ASCII 标识**。
 */
export function normalizeBloomLevel(value: unknown, fallback: BloomLevel = 'remember'): BloomLevel {
  ...
  // 常见别名容错
  const alias: Record<string, BloomLevel> = {
    记忆: 'remember',
    理解: 'understand',
    应用: 'apply',
    分析: 'analyze',
...
export function bloomAtLeast(level: BloomLevel, floor: BloomLevel): boolean {
  return BLOOM_LEVELS.indexOf(level) >= BLOOM_LEVELS.indexOf(floor)
}

/** 学习活动模式。 */
export type ActivityMode = 'teaching' | 'peer-exchange' | 'guided-reading'
...
export const TEACHING_SCENARIO_LABEL: Record<TeachingScenario, string> = {
  general: '通用',
  regulation: '条例学习',
  law: '法律学习',
  'course-concept': '概念学习',
...
/** 追问深度状态。 */
export type ProbeDepth = 0 | 1 | 2 | 3

export const PROBE_DEPTH_MAX: ProbeDepth = 3

/** 最近一条证据的语气。 */
export type EvidenceTone = 'unknown' | 'confused' | 'partial' | 'mastered'
