...
export type MetricSource = 'evidence' | 'derived' | 'heuristic'
...
export type Metric<T = number> = {
  ...
  /** 启发式指标必须给出解释，说明它为什么只是估计。 */
  readonly note?: string
  /** 直接支撑该数字的证据条数（`evidence` / `derived` 必填）。 */
  readonly basedOn?: number
}

export function metric<T>(value: T, source: MetricSource, extra: { note?: string; basedOn?: number } = {}): Metric<T> {
...
export type FlashcardLike = {
  readonly id: string
  readonly reviewHistory?: readonly { readonly reviewedAt: string; readonly result: string }[]
  readonly srs?: SrsState
}

export type StudyStats = {
  readonly totalNodes: Metric
  readonly masteredNodes: Metric
  readonly partialNodes: Metric
  readonly confusedNodes: Metric
  readonly totalEvidence: Metric
  readonly metacognitionCount: Metric
  readonly repeatedConfusedNodes: Metric
  readonly reviewDays: Metric
  readonly dueCards: Metric
  readonly reviewedCards: Metric
}
...
export function buildStudyStats(input: {
  ...
  flashcards: readonly FlashcardLike[]
  /** 该教材范围内的全部证据；其中 `srs-review` 行是复习统计的一手来源。 */
  reviewEvidence?: readonly Evidence[]
  now?: Date
}): StudyStats {
  ...
  for (const node of studyNodes) {
    ...
    const mastery: Mastery = masteryFromEvidence(items)
...
function isCardDue(card: FlashcardLike, now: Date): boolean {
 ...
 */
export function renderStats(stats: StudyStats): string {
  const rows: [string, Metric][] = [
