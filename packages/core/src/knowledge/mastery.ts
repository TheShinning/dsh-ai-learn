import type { BloomLevel, Mastery } from '../types.ts'
import { type Evidence, orderEvidence } from './evidence.ts'
...
const STRUCTURAL_KINDS: ReadonlySet<Evidence['kind']> = new Set([
  'note',
  'blackboard',
  'code-lab',
  'flashcard',
 ...
 */
export function isPositiveEvidence(evidence: Evidence): boolean {
...
/** 正向证据覆盖了多少个**不同**种类。 */
export function positiveKindCount(evidence: readonly Evidence[]): number {
 ...
 */
export function masteryFromEvidence(evidence: readonly Evidence[]): Mastery {
  const ordered = orderEvidence(evidence)
  let state: Mastery = 'unknown'
 ...
 */
export function deriveMastery(history: readonly Evidence[], incoming: Evidence): Mastery {
 ...
 * **任何**证据都能撤销，不需要快照。
 */
export function masteryWithout(evidence: readonly Evidence[], removedId: string): Mastery {
 ...
 */
export function suggestBloomLevel(understandingSignals: number, anchorSignals: number): BloomLevel {
 ...
 * @param floor 所需的最低层级。
 */
export function hasReachedBloom(evidence: readonly Evidence[], floor: BloomLevel = 'apply'): boolean {
