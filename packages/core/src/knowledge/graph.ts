...
import { MASTERY_RANK } from '../types.ts'
import type { Evidence } from './evidence.ts'
import { masteryFromEvidence } from './mastery.ts'

/** 节点类型。原系统只有 textbook/chapter/section/page 四种，且**没有建节点 API**。 */
export type NodeType = 'textbook' | 'chapter' | 'section' | 'page' | 'concept'

/** 边关系。原系统 `learning_knowledge_graph.go:591-598,832-834`。 */
export type EdgeRelation = 'contains' | 'prerequisite' | 'contrasts' | 'confused-with' | 'example-of'
...
export type MasteryIndex = Readonly<Record<string, Mastery>>

export function buildMasteryIndex(graph: KnowledgeGraph, nodeIds?: readonly string[]): MasteryIndex {
  ...
  const index: Record<string, Mastery> = {}
 ...
 * 层级信息本来就随证据进来。
 */
export function nodeBloomLevel(graph: KnowledgeGraph, nodeId: string): string | undefined {
...
export function routeScore(input: {
  ...
  prerequisites: readonly { node: KnowledgeNode; mastery: Mastery }[]
}): RouteStop {
...
function nodeSortKey(node: KnowledgeNode): string {
...
const CONTAINER_NODE_TYPES: ReadonlySet<NodeType> = new Set<NodeType>(['textbook', 'chapter'])
...
export function buildRouteStops(
  ...
  options: { limit?: number; textbookKey?: string; includeContainers?: boolean } = {},
): RouteStop[] {
  ...
  const stops = nodes.map((node) => {
    const prerequisites = graph.edges
      ...
      .filter((candidate): candidate is KnowledgeNode => candidate !== undefined)
...
}, existing: KnowledgeGraph): KnowledgeGraph {
...
export function masteryStats(graph: KnowledgeGraph, masteryIndex: MasteryIndex): {
  total: number
  mastered: number
  partial: number
  confused: number
...
export function prerequisitesReady(graph: KnowledgeGraph, masteryIndex: MasteryIndex, nodeId: string): boolean {
  return graph.edges
