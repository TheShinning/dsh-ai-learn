import type { BloomLevel, ErrorType, EvidenceTone, Mastery, ProbeDepth } from '../types.ts'
...
export type Evidence = {
  ...
  /** 结果标记，例如 `light` / `consolidate` / `clarify` / `run` / `ask`。 */
  readonly result?: string
  readonly confidence?: number
  /** 显式掌握度：一旦给出，直接覆盖推导（原系统优先级最高的一支）。 */
  readonly mastery?: Mastery
  /** 仅 `mastery-confirm` 携带：撤销时用于回放的基线。 */
  readonly previousMastery?: Mastery
  /** 三态：`undefined` 表示"未作答"，`false` 表示"答错"。 */
  readonly correct?: boolean
  readonly bloomLevel?: BloomLevel
  readonly srsResult?: SrsResult
  readonly errorType?: ErrorType
  /** 该条证据落在了哪个问题/追问深度上（`question-asked` 使用）。 */
 ...
 */
export function evidenceKey(input: Pick<Evidence, 'nodeId' | 'kind' | 'sourceId' | 'summary'>): string {
...
export function stableEvidenceDigest(input: string): string {
  let hash = 0x811c9dc5
...
export class EvidenceIdAllocator {
   ...
   */
  next(key: string, at: Date = new Date()): string {
...
export function evidenceTone(evidence: Evidence | undefined): EvidenceTone {
