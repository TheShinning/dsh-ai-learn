...
export const SRS_RESULT_LABEL: Record<SrsResult, string> = {
  again: '错了',
  hard: '勉强对',
  good: '答对了',
}
...
export function dayKey(at: Date | number | string): string {
 ...
 * 误差而提前或推迟一天。
 */
export function nextReviewAt(from: Date, intervalDays: number): string {
 ...
 */
export function scheduleReview(state: SrsState, result: SrsResult, at: Date = new Date()): SrsState {
 ...
 */
export function srsMasterySignal(result: SrsResult): 'confused' | 'partial' | undefined {
