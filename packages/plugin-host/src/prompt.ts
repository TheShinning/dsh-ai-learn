...
import type { ProbeState } from '../../core/src/pedagogy/probe.ts'
import { depthDirective, toneDirective } from '../../core/src/pedagogy/probe.ts'
import {
  SOCRATIC_ACTION_LABEL,
  SOCRATIC_STEP_LABEL,
  type SocraticAction,
  type SocraticStep,
} from '../../core/src/pedagogy/step.ts'
import { TEACHING_MODE_LABEL, type TeachingMode } from '../../core/src/pedagogy/strategy.ts'
import type { ErrorType, TeachingScenario } from '../../core/src/types.ts'
import { ERROR_TYPE_LABEL, TEACHING_SCENARIO_LABEL } from '../../core/src/types.ts'
...
export type StudyContextInput = {
  ...
  errorType?: ErrorType
  hasConfusedNodes?: boolean
  dueCards?: number
  /** 当前苏格拉底环节（`SocraticStep`）。 */
  step?: string
  /** 下一步动作（`SocraticAction`）。 */
  nextAction?: string
  /** 当前教学模式（`TeachingMode`）。 */
  teachingMode?: string
  /** 材料来源格式（pdf / docx / markdown …）。 */
  sourceFormat?: string
}
 ...
 */
export const teachingStateSnapshot: { current: StudyContextInput } = { current: {} }
...
export function buildStudyContext(input: StudyContextInput): string {
  ...
  if (input.sourceFormat || input.teachingMode) {
    ...
    if (input.teachingMode) parts.push(`教学模式 ${TEACHING_MODE_LABEL[input.teachingMode as TeachingMode] ?? input.teachingMode}`)
  ...
  if (input.step || input.nextAction) {
    const step = input.step ? (SOCRATIC_STEP_LABEL[input.step as SocraticStep] ?? input.step) : ''
    const action = input.nextAction ? (SOCRATIC_ACTION_LABEL[input.nextAction as SocraticAction] ?? input.nextAction) : ''
...
function errorTypeDirective(errorType: ErrorType): string {
  switch (errorType) {
