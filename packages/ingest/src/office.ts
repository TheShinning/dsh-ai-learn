...
import type { ParserCapability, SourceFormat, SourceParser } from '../../core/src/ingest/types.ts'
import { decodeEntities, decodeText, htmlToMarkdown, tidyMarkdown } from './text.ts'
...
function readU64(view: DataView, offset: number): number | undefined {
...
function decodeZipName(bytes: Uint8Array, _flags: number): string {
  try {
...
function findEocd(view: DataView): number | undefined {
...
type Zip64Sizes = { uncompressedSize?: number; compressedSize?: number; localHeaderOffset?: number }

/** 读 ZIP64 扩展字段（0x0001）：字段按「溢出的那一项」顺序排列。 */
function parseZip64Extra(bytes: Uint8Array, needSizes: boolean, needOffset: boolean): Zip64Sizes {
  const view = viewOf(bytes)
  let cursor = 0
  const result: Zip64Sizes = {}
...
function readCentralDirectory(bytes: Uint8Array, view: DataView, eocd: number): ZipEntry[] {
...
function scanLocalHeaders(bytes: Uint8Array, view: DataView): ZipEntry[] {
  const entries: ZipEntry[] = []
  let cursor = 0
...
/** 解压单个条目。 */
export function readZipEntryData(bytes: Uint8Array, entry: ZipEntry): Uint8Array {
  const view = viewOf(bytes)
...
function findZipEntry(entries: readonly ZipEntry[], name: string): ZipEntry | undefined {
...
export function readZipFile(bytes: Uint8Array, name: string): Uint8Array | undefined {
...
export function sniffOfficeFormat(names: readonly string[]): SourceFormat | undefined {
...
function headingLevelFromStyle(value: string): number {
  if (!value) return 0
 ...
 */
export function xmlToMarkdown(xml: string, dialect: XmlDialect): string {
  const parts: string[] = []
  ...
  const inTable = (): boolean => frames.some((frame) => frame.table)
  const currentParagraph = (): XmlFrame | undefined => {
  ...
  const currentTable = (): XmlFrame | undefined => {
    for (let index = frames.length - 1; index >= 0; index -= 1) {
      const frame = frames[index]!
  ...
  const popFrame = (name: string): XmlFrame | undefined => {
  ...
  const paragraphBreak = (): string => (inTable() ? '\n' : '\n\n')
...
export function docxToMarkdown(bytes: Uint8Array): { markdown: string; title?: string; author?: string; partial: boolean; reason?: string } {
...
export function odtToMarkdown(bytes: Uint8Array): { markdown: string; title?: string; author?: string; partial: boolean; reason?: string } {
  const entries = readZipEntries(bytes)
...
type EpubManifest = { readonly items: Map<string, string>; readonly properties: Map<string, string>; readonly spine: string[] }

function parseOpf(opf: string): EpubManifest & { title?: string; author?: string } {
...
export function epubToMarkdown(bytes: Uint8Array): { markdown: string; title?: string; author?: string; partial: boolean; reason?: string } {
  const entries = readZipEntries(bytes)
  ...
  let chapters = manifest.spine
    ...
    .filter((item): item is { id: string; href: string } => item !== undefined)
  if (!chapters.length) {
...
function containerCapability(id: string, format: SourceFormat): ParserCapability {
  return { id, tier: 'builtin', formats: [format], handlesScanned: false, available: true }
}

/** docx 解析器（builtin 层）。 */
export function docxParser(): SourceParser {
  return {
...
/** odt 解析器（builtin 层）。 */
export function odtParser(): SourceParser {
  return {
...
/** epub 解析器（builtin 层）。 */
export function epubParser(): SourceParser {
  return {
