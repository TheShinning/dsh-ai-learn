...
import type { ParserCapability, SourceParser } from '../../core/src/ingest/types.ts'
import { decodeText, tidyMarkdown } from './text.ts'
 ...
 */
export function commandAvailable(command: string, args: readonly string[] = ['-v']): boolean {
...
/** 清空探测缓存（测试用）。 */
export function resetCommandProbeCache(): void {
  probeCache.clear()
}
 ...
 */
export function runExternalCommand(command: string, args: readonly string[], timeoutMs = 180_000): ExternalRun {
  ...
  let result: ExternalRun = { ok: false, status: null, stdout: '', stderr: '', command: display, spawnError: '未执行' }
  try {
...
/** 临时目录句柄。 */
export type WorkDir = { readonly dir: string; dispose(): void }
...
export type MaterializedInput = { readonly file: string; readonly temporary: boolean; dispose(): void }

/**
 * 外部命令需要真实路径，而 `SourceParser.parse` 只拿到 bytes。
 * 已经是磁盘文件就直接用，否则写进临时目录。
 */
export function materializeInput(input: { bytes: Uint8Array; fileName: string }): MaterializedInput {
...
type CollectedStream = { readonly dict: string; readonly data: string }

/** 收集 `stream ... endstream` 段。按规范 `stream` 后必须紧跟 EOL，借此排除二进制里的假信号。 */
function collectStreams(raw: string): CollectedStream[] {
  const streams: CollectedStream[] = []
  let cursor = 0
...
function inflateMaybe(buffer: Buffer): Buffer | undefined {
  try {
...
type PdfToken =
  ...
  | { readonly kind: 'array-open' }
  | { readonly kind: 'array-close' }
  | { readonly kind: 'dict-open' }
  | { readonly kind: 'dict-close' }

function isPdfWhitespace(char: string): boolean {
...
function readLiteralString(source: string, start: number): { bytes: Uint8Array; next: number } {
  const out: number[] = []
  let depth = 1
  let i = start + 1
...
function readHexString(source: string, start: number): { bytes: Uint8Array; next: number } {
  const out: number[] = []
  let digits = ''
  let i = start + 1
...
function tokenizePdf(source: string): PdfToken[] {
...
function decodeStringToken(token: PdfToken | undefined): string {
  if (!token || token.kind !== 'string') return ''
  return decodeText(token.bytes).text
}

function lastString(operands: readonly PdfToken[]): PdfToken | undefined {
...
function lastArrayElements(operands: readonly PdfToken[]): PdfToken[] {
  let close = -1
  for (let i = operands.length - 1; i >= 0; i -= 1) {
...
function numericOperands(operands: readonly PdfToken[]): number[] {
  return operands.filter((token): token is { kind: 'number'; value: number } => token.kind === 'number').map((token) => token.value)
}
...
export function extractTextFromContentStream(content: string): string {
  ...
  let lastMatrixY: number | undefined
 ...
 */
export function extractPdfTextLayer(bytes: Uint8Array): PdfTextLayerResult {
...
export function pdfBuiltinParser(): SourceParser {
...
export type PdfExternalOptions = {
  readonly command?: string
 ...
 */
export function pdfPopplerParser(options: PdfExternalOptions = {}): SourceParser {
...
export type PdfOcrOptions = {
  readonly command?: string
  readonly commandArgs?: readonly string[]
  readonly rasterCommand?: string
  readonly rasterArgs?: readonly string[]
  readonly languages?: string
  readonly fallbackLanguages?: string
  readonly dpi?: number
  readonly maxPages?: number
  readonly timeoutMs?: number
}

function naturalPageOrder(name: string): number {
 ...
 * `reason` 里记录实际执行的两条命令。
 */
export function pdfOcrParser(options: PdfOcrOptions = {}): SourceParser {
  ...
  const probe = (spec: string, args: readonly string[]): boolean => commandAvailable(spec, [...args, '-v'])
  ...
  return {
    ...
    parse: async (input) => {
      ...
      try {
        ...
        const usedCommands: string[] = [raster.command]
        const chunks: string[] = []
