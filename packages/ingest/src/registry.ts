...
import { imageCapability, readImageBytes } from './image.ts'
import { docxParser, epubParser, odtParser, readZipEntries, sniffOfficeFormat } from './office.ts'
import { pdfBuiltinParser, pdfOcrParser, pdfPopplerParser } from './pdf.ts'
import { htmlParser, markdownParser, plainTextParser, textFallbackParser } from './text.ts'

const TIER_ORDER: readonly IngestTier[] = ['native', 'builtin', 'external']

/** 稳定按 tier 排序（同级保持声明顺序），不依赖 sort 的稳定性直觉。 */
function orderByTier<T>(items: readonly T[], tierOf: (item: T) => IngestTier): T[] {
  return items
...
function buildParsers(): SourceParser[] {
  return orderByTier<SourceParser>(
    [
      // native：运行时直接能吃
      markdownParser(),
      plainTextParser(),
...
let cachedParsers: SourceParser[] | undefined

/** 组装好的解析器（按 tier 有序）。顺序即降级顺序。 */
export function defaultParsers(): Promise<SourceParser[]> {
...
export async function capabilities(): Promise<ParserCapability[]> {
 ...
 */
export function sniffContainerFormat(bytes: Uint8Array): SourceFormat | undefined {
  try {
...
export async function ingestBytes(input: { bytes: Uint8Array; fileName: string; reference?: string }): Promise<IngestResult | ImageIngestResult> {
...
export async function ingestFile(filePath: string): Promise<IngestResult | ImageIngestResult> {
