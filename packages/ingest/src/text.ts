...
import type { IngestTier, ParserCapability, SourceFormat, SourceParser } from '../../core/src/ingest/types.ts'
...
function tryDecode(label: string, bytes: Uint8Array, fatal: boolean): string | undefined {
  try {
 ...
 */
function looksLikeGbk(bytes: Uint8Array, decoded: string): boolean {
  let nonAscii = 0
...
function guessBomlessUtf16(bytes: Uint8Array): 'utf-16le' | 'utf-16be' | undefined {
...
function collapseHtmlWhitespace(input: string): string {
...
export function decodeEntities(input: string): string {
  return input.replace(/&(#[xX]?[0-9a-fA-F]+|[a-zA-Z][a-zA-Z0-9]{0,31});/g, (match, body: string) => {
    if (body.startsWith('#')) {
...
function deferEntityDecode(input: string): string {
...
export function htmlToMarkdown(html: string): string {
  ...
  source = source.replace(/<head[^>]*>[\s\S]*?<\/head\s*>/gi, (head: string) => {
    const titles: string[] = []
  ...
  source = source.replace(/<title[^>]*>([\s\S]*?)<\/title\s*>/gi, (_match, inner: string) => {
  ...
  source = source.replace(/<ol\b[^>]*>([\s\S]*?)<\/ol\s*>/gi, (_match, inner: string) => {
    let index = 0
  ...
  source = source.replace(/<a\b([^>]*)>([\s\S]*?)<\/a\s*>/gi, (_match, attributes: string, inner: string) => {
    const text = decodeEntities(stripRemainingTags(inner)).replace(/\s+/g, ' ').trim()
  ...
  source = source.replace(/<img\b([^>]*)\/?>/gi, (_match, attributes: string) => {
...
function capabilityOf(id: string, tier: IngestTier, formats: SourceFormat[], handlesScanned = false): ParserCapability {
  return { id, tier, formats, handlesScanned, available: true }
}

/** 解析 YAML front matter 里的 title/author（只认最简单的 `key: value`）。 */
export function parseFrontMatter(markdown: string): { title?: string; author?: string; body: string } {
...
function firstLine(markdown: string, maxLength = 120): string | undefined {
  for (const line of markdown.split('\n')) {
...
export function markdownParser(): SourceParser {
  return {
...
/** 纯文本直通解析（native 层）。 */
export function plainTextParser(): SourceParser {
  return {
...
export function htmlParser(): SourceParser {
  return {
 ...
 */
export function textFallbackParser(): SourceParser {
  return {
